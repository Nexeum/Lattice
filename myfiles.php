    ...
    <div class="content-wrapper"><!-- Content Wrapper. Contains page content -->
        <section class="content-header"><!-- Content Header (Page header) -->
            ...
            <ol class="breadcrumb">
                ...
                <?php
                    if (($folder) && (mysqli_num_rows($folder) != 0)) {
        ...
        <section class="content"><!-- Main content -->
            <div class="row"><!-- Small boxes (Stat box) -->
                <div class="col-md-12">
                <?php
                        $files = mysqli_query($con, "select * from file where user_id = $my_user_id");
                            if (($folder) && (mysqli_num_rows($folder) == 0)) {
                                if (isset($_GET["q"]) && $_GET["q"] != "") {
                                    $q = $_GET["q"];
                                    $files = mysqli_query($con, "select * from file where user_id=$my_user_id and folder_id is NULL and (filename like '%$q%' or description like '%$q%') order by is_folder desc, created_at desc");
                                } else {
                                    $files = mysqli_query($con, "select * from file where user_id=$my_user_id and folder_id is NULL order by is_folder desc, created_at desc");
                                }
                            } else {
                                // search
                ...
                <?php endif; ?>
                    ...
                    <div class="box">
                        <div class="box-header">
                            <?php if(($folder) && mysqli_num_rows($folder)==0):?>
